...
static inline cputime64_t sget_cpu_idle_time_jiffy(unsigned int cpu,
                                                  cputime64_t *wall)
{
        u64 idle_time;
        u64 cur_wall_time;
...
static inline cputime64_t sget_cpu_idle_time(unsigned int cpu,
                                            cputime64_t *wall)
{
        u64 idle_time = get_cpu_idle_time_us(cpu, wall);

        if (idle_time == -1ULL)
                idle_time = sget_cpu_idle_time_jiffy(cpu, wall);        
...
static void cpufreq_ondemandplus_timer(unsigned long data)
{
        ...
        now_idle = sget_cpu_idle_time(data, &pcpu->timer_run_time);
        smp_wmb();
...
rearm:
        if (!timer_pending(&pcpu->cpu_timer)) {
                ...
                pcpu->time_in_idle = sget_cpu_idle_time(
...
static void cpufreq_ondemandplus_idle_end(void)
{
        ...
        if (timer_pending(&pcpu->cpu_timer) == 0 &&
            ...
            pcpu->governor_enabled) {
                pcpu->time_in_idle =
                        sget_cpu_idle_time(smp_processor_id(),
                                             &pcpu->idle_exit_time);
                pcpu->timer_idlecancel = 0;
                mod_timer(&pcpu->cpu_timer,
                          jiffies + usecs_to_jiffies(timer_rate));
