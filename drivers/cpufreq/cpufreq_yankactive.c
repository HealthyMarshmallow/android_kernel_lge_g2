...
static inline cputime64_t uget_cpu_idle_time_jiffy(unsigned int cpu,
						  cputime64_t *wall)
{
	u64 idle_time;
	u64 cur_wall_time;
...
static inline cputime64_t uget_cpu_idle_time(unsigned int cpu,
					    cputime64_t *wall)
{
	u64 idle_time = get_cpu_idle_time_us(cpu, wall);

	if (idle_time == -1ULL)
		idle_time = uget_cpu_idle_time_jiffy(cpu, wall);
	else if (!io_is_busy)
	...
	struct cpufreq_yankactive_cpuinfo *pcpu)
{
	...
	pcpu->time_in_idle =
		uget_cpu_idle_time(smp_processor_id(),
...
static void cpufreq_yankactive_timer_start(int cpu, int time_override)
{
	...
	spin_lock_irqsave(&pcpu->load_lock, flags);
	pcpu->time_in_idle =
		uget_cpu_idle_time(cpu, &pcpu->time_in_idle_timestamp);
	pcpu->cputime_speedadj = 0;
	pcpu->cputime_speedadj_timestamp = pcpu->time_in_idle_timestamp;
	spin_unlock_irqrestore(&pcpu->load_lock, flags);
}
...
static u64 update_load(int cpu)
{
	...
	now_idle = uget_cpu_idle_time(cpu, &now);
