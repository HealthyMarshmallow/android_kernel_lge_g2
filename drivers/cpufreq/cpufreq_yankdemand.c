...
static inline u64 uget_cpu_idle_time_jiffy(unsigned int cpu, u64 *wall)
{
	u64 idle_time;
	u64 cur_wall_time;
	u64 busy_time;
...
static inline cputime64_t uget_cpu_idle_time(unsigned int cpu, cputime64_t *wall)
{
	u64 idle_time = get_cpu_idle_time_us(cpu, NULL);

	if (idle_time == -1ULL)
		return uget_cpu_idle_time_jiffy(cpu, wall);
	else
...
static inline cputime64_t uget_cpu_iowait_time(unsigned int cpu, cputime64_t *wall)
{
				      ...
				      const char *buf, size_t count)
{
	...
	for_each_online_cpu(j) {
		struct cpu_dbs_info_s *dbs_info;
		dbs_info = &per_cpu(od_cpu_dbs_info, j);
		dbs_info->prev_cpu_idle = uget_cpu_idle_time(j,
...
/* PATCH : SMART_UP */
	...
	for_each_cpu(j, policy->cpus) {
		...
		cur_idle_time = uget_cpu_idle_time(j, &cur_wall_time);
		cur_iowait_time = uget_cpu_iowait_time(j, &cur_wall_time);
...
static void dbs_refresh_callback(struct work_struct *work)
{
	...
	if (policy->cur < target_freq) {
		...
		this_dbs_info->prev_cpu_idle = uget_cpu_idle_time(cpu,
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			j_dbs_info->prev_cpu_idle = uget_cpu_idle_time(j,
